...
{
    [ScriptType(name: "绝妖星P134面向P4动静剑辅助", territorys: [1363], guid: "d4e5f6a1-c2b3-49d8-90ef-1a2b3c4d5e6f", version: "0.1.3.2", note: "萌新第一次写脚本。测试版，包含P1P3面向，及P4真伪石化、真伪炸弹自动处理，可能会电。类似功能的脚本尽量只开一个。\n依赖于DailyRoutines插件中的自动面向摄像机方向（在线验证模块）和禁止主动移动与释放技能（自制本地模块）。\n想要稳定使用的话请麻烦去可达鸭dc示例和分享中[月猫三]个人脚本分享里看下使用说明。", author: "yuemao3")]
    public class DMU_AutoFaceCombinedAssist
    {
        ...
        private void InitDRIpc(ScriptAccessory sa)
        {
            try 
            {
                ...
                _drSub3 = m2.MakeGenericMethod(typeof(bool), typeof(object)).Invoke(pi, new object[] { "DailyRoutines.Modules.ForceRootAndSilence.SetRootAndSilence_V5" });
        ...
        public void Init(ScriptAccessory accessory)
        {
            ...
            Task.Run(async () => {
                ...
                if (hasRoot) 
                {
                    try { if (_drActionSetRoot == null) InitDRIpc(accessory); _drActionSetRoot?.Invoke(false); } 
                    catch { isV5 = false; }
                }

                if (!isVerified || !hasRoot || !isV5) {
                    string missing = "";
                    if (!isVerified) missing += " [未通过 DR 在线验证] ";
                    if (!hasRoot) missing += " [缺少或未开启 ForceRootAndSilence 本地模块] ";
                    if (hasRoot && !isV5) missing += " [检测到旧版 ForceRootAndSilence 本地模块] ";
                    
                    accessory.Method.SendChat($"/e [绝妖星面向辅助 报错] 自检失败：{missing}");
                    
                    if (hasRoot && !isV5) {
                        accessory.Method.SendChat("/e [绝妖星面向辅助 报错] 游戏版本更新已导致旧模块失效！请前往可达鸭discord月猫三的脚本分享，下载并导入最新版（第五版）的本地模块。");
                    } else {
                        accessory.Method.SendChat("/e [绝妖星面向辅助 报错] 请确保DR已开启并完成在线验证，且正确导入了配套本地模块。");
                    }
