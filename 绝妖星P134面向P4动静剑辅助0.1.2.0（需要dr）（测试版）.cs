...
{
    [ScriptType(name: "绝妖星P134面向P4动静剑辅助", territorys: [1363], guid: "d4e5f6a1-c2b3-49d8-90ef-1a2b3c4d5e6f", version: "0.1.2.0", note: "萌新第一次写脚本。测试版，包含P1P3面向，及P4真伪石化、真伪炸弹自动处理，可能会电。类似功能的脚本尽量只开一个。\n依赖于DailyRoutines插件中的自动面向摄像机方向（在线验证模块）和禁止主动移动与释放技能（自制本地模块）。\n想要稳定使用的话请麻烦去可达鸭dc示例和分享中[月猫三]个人脚本分享里看下使用说明。", author: "yuemao3")]
    public class FRU_AutoFaceCombinedAssist
    {
        ...
        private Dictionary<uint, long> _petrifyExpirations = new Dictionary<uint, long>();

        // 全局防闪断阈值 (统一乘以 50ms 延时 = 0.5秒)
        ...
        private void InitDRIpc(ScriptAccessory sa)
        {
            try 
            {
                ...
                if (pi == null) 
                {
                    sa.Method.SendChat("/e [IPC报错] 请打开DR并进行在线验证，且确保加载了配套本地模块。");
                    return;
                }
                ...
                if (_drSubIsMod == null || _drInvIsMod == null ||
                    _drSubLoad == null || _drInvLoad == null ||
                    _drSubUnload == null || _drInvUnload == null ||
                    _drSub1 == null || _drInv1 == null ||
                    _drSub2 == null || _drInv2 == null ||
                    _drSub3 == null || _drInv3 == null ||
                    _drSub4 == null || _drInv4 == null ||
                    _drSub6 == null || _drInv6 == null)
                {
                    sa.Method.SendChat("/e [IPC报错] 请打开DR并进行在线验证，且确保加载了配套本地模块。");
                    return;
                }
                ...
                try 
                {
                    _drInvIsMod.Invoke(_drSubIsMod, new object[] { "PingTest_CheckIfAlive" });
                }
                catch 
                {
                    sa.Method.SendChat("/e [IPC报错] 未检测到DR运行！请打开DR并完成在线验证，且确保加载了配套本地模块。");
        ...
        {
            try {
                if (_drSubIsMod == null) InitDRIpc(sa);
        ...
        {
            try { if (_drSubLoad == null) InitDRIpc(sa); _drInvLoad?.Invoke(_drSubLoad, new object[] { moduleName, false }); } catch { }
        }

        private void DR_UnloadModule(ScriptAccessory sa, string moduleName)
        {
            try { if (_drSubUnload == null) InitDRIpc(sa); _drInvUnload?.Invoke(_drSubUnload, new object[] { moduleName, false, true }); } catch { }
        }
        ...
        private async Task<bool> EnsureModuleLoaded(ScriptAccessory sa, string moduleName, int currentRunId)
        {
            ...
            sa.Method.SendChat($"/e [IPC报错] DR 模块 {moduleName} 加载超时/失败。");
            return false;
        }

        private void DR_LockOnChara(ScriptAccessory sa, float r) { 
            try { if (!DR_IsModuleEnabled(sa, "AutoFaceCameraDirection")) return; if (_drSub1 == null) InitDRIpc(sa); _drInv1?.Invoke(_drSub1, new object[] { r }); } catch { } 
        }
        private void DR_CancelLockOn(ScriptAccessory sa) { 
            try { if (!DR_IsModuleEnabled(sa, "AutoFaceCameraDirection")) return; if (_drSub2 == null) InitDRIpc(sa); _drInv2?.Invoke(_drSub2, null); } catch { } 
        }
        private void DR_SetRootAndSilence(ScriptAccessory sa, bool s) { 
            try { if (!DR_IsModuleEnabled(sa, "ForceRootAndSilence")) return; if (_drSub3 == null) InitDRIpc(sa); _drInv3?.Invoke(_drSub3, new object[] { s }); } catch { } 
        }
        private void DR_TriggerJump(ScriptAccessory sa) { 
            try { if (!DR_IsModuleEnabled(sa, "ForceRootAndSilence")) return; if (_drSub4 == null) InitDRIpc(sa); _drInv4?.Invoke(_drSub4, null); } catch { } 
        }
        private void DR_SetAutoFaceTarget(ScriptAccessory sa, bool s) { 
            try { if (!DR_IsModuleEnabled(sa, "ForceRootAndSilence")) return; if (_drSub6 == null) InitDRIpc(sa); _drInv6?.Invoke(_drSub6, new object[] { s }); } catch { } 
        }
        ...
        private void LogDebug(ScriptAccessory sa, string message) { if (EnableDebugLog) sa.Method.SendChat($"/e [测试日志] {message}"); }
        
        private void NotifyNullBreak(ScriptAccessory sa) { sa.Method.SendChat("/e [机制报错] 检测到长时间失去玩家对象内存，触发严重异常熔断！自动脚本失效，请手动处理机制！"); }
        ...
        public void Init(ScriptAccessory accessory)
        {
            ...
            Task.Run(async () => {
                ...
                if (!origSwim) DR_LoadModule(accessory, "ProhibitSwimming"); 
                ...
                if (!isVerified || !hasRoot) {
                    ...
                    if (!hasRoot) missing += " [缺少 ForceRootAndSilence 本地模块] ";
                    accessory.Method.SendChat($"/e [IPC报错] 自检失败：{missing}\n请确保DR已开启并完成在线验证，且正确导入了配套本地模块。");
        ...
        {
            while (rot > MathF.PI) rot -= 2 * MathF.PI;
            while (rot < -MathF.PI) rot += 2 * MathF.PI;
        ...
        private float CalcBossTrackingRot(ScriptAccessory sa, uint bossId)
        {
            ...
            var boss = (bossId != 0 ? sa.Data.Objects.SearchById(bossId) : null) ?? sa.Data.Objects.FirstOrDefault(o => o.DataId == 18475);
        ...
        private float CalcPlayerTargetRot(ScriptAccessory sa)
        {
            var myObj = sa.Data.MyObject;
            if (myObj == null) return 0.001f;
            uint targetId = (uint)myObj.TargetObjectId;
            if (targetId != 0 && targetId != 0xE0000000) {
                var target = sa.Data.Objects.SearchById(targetId);
                if (target != null) { return NormalizeRot(MathF.Atan2(target.Position.X - myObj.Position.X, target.Position.Z - myObj.Position.Z)); }
            }
            return NormalizeRot(myObj.Rotation);
        }
        ...
        public async void P1_AutoFace_Execute(Event @event, ScriptAccessory accessory)
        {
            ...
            if (actualSecondWait > 0) {
                ...
                if (rem > 0) {
                    var pState = CheckPlayerState(accessory, ref nullRetryCount, currentRunId);
                    if (pState == PlayerState.Valid) { DR_LockOnChara(accessory, CalcBossTrackingRot(accessory, bossId)); }
        ...
        public async void VacuumWave_StartAssist(Event ev, ScriptAccessory sa)
        {
            ...
            int elapsed = (int)(DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() - loadStart);
            int actualSecondWait = Math.Max(0, secondWait - elapsed);
            float initSoftRot = CalcPlayerTargetRot(sa);
            LogDebug(sa, $"P3 进入缓冲软锁定，持续 {actualSecondWait}ms。快照锁定玩家当前目标/面向角度: {FormatAngle(initSoftRot)}");
            ...
            if (actualSecondWait > 0) {
                ...
                for (int i = 0; i < preLoops; i++) {
                    ...
                    if (pState == PlayerState.Abort) return;
                    if (pState == PlayerState.Retry) { await Task.Delay(50); continue; }
                    DR_LockOnChara(sa, CalcPlayerTargetRot(sa));
                    await Task.Delay(100);
                }
                int rem = actualSecondWait % 100;
                if (rem > 0) {
                    var pState = CheckPlayerState(sa, ref nullRetryCount, currentRunId);
                    if (pState == PlayerState.Valid) DR_LockOnChara(sa, CalcPlayerTargetRot(sa));
                    await Task.Delay(rem);
                }
            }
            if (currentRunId != _runId) return;
            ...
            try {
                _isLocking = true;
                nullRetryCount = 0;
                for (int i = 0; i < 90 && _isLocking && currentRunId == _runId; i++) {
                    var pState = CheckPlayerState(sa, ref nullRetryCount, currentRunId);
                    if (pState == PlayerState.Abort) break;
                    if (pState == PlayerState.Retry) { await Task.Delay(50); continue; }
        ...
        public void P4_DebuffExecute(Event @event, ScriptAccessory sa)
        {
            ...
            if (@event["StatusID"] == "5543") {
                ...
                uint targetId = (uint)@event.TargetId;
                long expTime = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + (long)dur;
            ...
            if (@event["StatusID"] == "5543" && EnableP4Petrify) {
                ...
                _ = HandleP4PetrifyAsync(sa, isTrue, (int)dur, _runId, 0); 
            }
        ...
        private float CalcP4PetrifySafeRot(ScriptAccessory sa, bool isTrue, uint bossId, List<uint> targetPlayerIds)
        {
            var myObj = sa.Data.MyObject;
            if (myObj == null) return 0.001f;
        ...
        private async Task HandleP4PetrifyAsync(ScriptAccessory sa, bool isTrue, int dur, int runId, uint bossId)
        {
            ...
            List<uint> cachedPetrifyPlayerIds = new List<uint>();
            LogDebug(sa, $"[花名册检索] 机制即将结算！目标时间戳: {targetTime} (容差±2000ms)");
            uint myEntityId = sa.Data.MyObject?.EntityId ?? 0;
            ...
            LogDebug(sa, $"[花名册确认] 初始筛选出存活且保有Buff的有效目标数量: {initialActivePlayers.Count} 名。开始追踪坐标！");
            ...
            try {
                ...
                while (DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() < releaseTime && _isLocking) {
                    ...
                    // 防判定期提前掉Buff：在最后的锁定时间里，不再用 HasStatusAny 去筛人，只剔除这零点几秒内意外死亡的队友
        ...
        private async Task HandleP4AccelBombRootAsync(ScriptAccessory sa, int dur, int runId)
        {
            ...
            try
            {
                if (EnableAeCompatibility) {
                    // 第 1 次关闭 AE (倒数4秒)
                    while (DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() < aeOffTime1) {
                        var pState = CheckPlayerState(sa, ref nullRetryCount, runId);
                        if (pState == PlayerState.Abort) return;
                        if (pState == PlayerState.Retry) { await Task.Delay(50); continue; }
                        if (!sa.Data.MyObject.HasStatusAny(new uint[] { 5546 })) return;
                        await Task.Delay(50);
                    }
                    LogDebug(sa, "准备停手：触发第1道安全锁，发送 /aeTargetSelector off");
                    sa.Method.SendChat("/aeTargetSelector off");
                    hasDisabledAe = true; 

                    // 第 2 次关闭 AE (倒数3秒)
                    while (DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() < aeOffTime2) {
                        var pState = CheckPlayerState(sa, ref nullRetryCount, runId);
                        if (pState == PlayerState.Abort) return;
                        if (pState == PlayerState.Retry) { await Task.Delay(50); continue; }
                        if (!sa.Data.MyObject.HasStatusAny(new uint[] { 5546 })) return;
                    ...
                    LogDebug(sa, "准备停手：触发第2道安全锁，发送 /aeTargetSelector off");
                    sa.Method.SendChat("/aeTargetSelector off");
                }
