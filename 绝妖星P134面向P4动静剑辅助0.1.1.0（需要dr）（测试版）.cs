...
{
    [ScriptType(name: "绝妖星P134面向P4动静剑辅助", territorys: [1363], guid: "d4e5f6a1-c2b3-49d8-90ef-1a2b3c4d5e6f", version: "0.1.1.0", note: "萌新第一次写脚本。测试版，包含P1P3面向，及P4真伪石化、真伪炸弹自动处理，可能会电。类似功能的脚本尽量只开一个。\n依赖于DailyRoutines插件中的自动面向摄像机方向（在线验证模块）和禁止主动移动与释放技能（自制本地模块）。\n想要稳定使用的话请麻烦去可达鸭dc示例和分享中[月猫三]个人脚本分享里看下使用说明。", author: "yuemao3")]
    public class FRU_AutoFaceCombinedAssist
    {
        ...
        public void Init(ScriptAccessory accessory)
        {
            ...
            _petrifyExpirations.Clear(); // 清空花名册
        ...
        private float CalcBossTrackingRot(ScriptAccessory sa, uint bossId)
        {
            ...
            var boss = (bossId != 0 ? sa.Data.Objects.SearchById(bossId) : null) 
                    ?? sa.Data.Objects.FirstOrDefault(o => o.DataId == 18475);
        ...
        private float CalcPlayerTargetRot(ScriptAccessory sa)
        {
            ...
            if (targetId != 0 && targetId != 0xE0000000) 
            {
                ...
                if (target != null)
                {
                    var rot = MathF.Atan2(target.Position.X - myObj.Position.X, target.Position.Z - myObj.Position.Z);
        ...
        private async Task HandleP4PetrifyAsync(ScriptAccessory sa, bool isTrue, int dur, int runId, uint bossId)
        {
            ...
            {
                if (runId != _runId || sa.Data.MyObject == null || sa.Data.MyObject.IsDead) return;
                await Task.Delay(50);
            }
            ...
            var expectedPlayerIds = _petrifyExpirations
                ...
                .Select(kv => kv.Key)
                .Where(id => id != sa.Data.MyObject.EntityId) 
                .ToList();

            LogDebug(sa, $"[花名册检索] 找到 {expectedPlayerIds.Count} 名预期在此波次结算的队友。");

            var activePlayersWithBuff = expectedPlayerIds
                .Where(id => {
            ...
            LogDebug(sa, $"[花名册确认] 最终筛选出存活且保有Buff的有效目标数量: {activePlayersWithBuff.Count} 名。开始追踪坐标！");
            // --- 测试日志结束 ---

            if (activePlayersWithBuff.Count > 0)
            {
                cachedPetrifyPlayerIds = activePlayersWithBuff;
            }
            ...
            try 
            {
                ...
                {
                    if (runId != _runId || sa.Data.MyObject == null || sa.Data.MyObject.IsDead) break;
                    
                    var currentActivePlayers = expectedPlayerIds
                        .Where(id => {
                            var obj = sa.Data.Objects.SearchById(id) as IBattleChara;
                            return obj != null && !obj.IsDead && obj.HasStatusAny(new uint[] { 5543 });
                        })
                    ...
                    {
                        cachedPetrifyPlayerIds = currentActivePlayers;
                    }

                    float currentRot = CalcP4PetrifySafeRot(sa, isTrue, bossId, cachedPetrifyPlayerIds);

                    DR_LockOnChara(sa, currentRot); 
                    await Task.Delay(50);
                }
            } 
            finally 
