...
{
    [ScriptType(name: "绝妖星P134面向P4动静剑辅助", territorys: [1363], guid: "d4e5f6a1-c2b3-49d8-90ef-1a2b3c4d5e6f", version: "0.1.0.6", note: "萌新第一次写脚本。测试版，包含P1P3面向，及P4真伪石化、真伪炸弹自动处理，可能会电。类似功能的脚本尽量只开一个。\n依赖于DailyRoutines插件中的自动面向摄像机方向（在线验证模块）和禁止主动移动与释放技能（自制本地模块）。\n想要稳定使用的话请麻烦去可达鸭dc示例和分享中[月猫三]个人脚本分享里看下使用说明。", author: "yuemao3")]
    public class FRU_AutoFaceCombinedAssist
    {
        ...
        public void Init(ScriptAccessory accessory)
        {
            ...
            Task.Run(async () => {
                // ================= 新增：在线验证与本地模块联合代理自检 =================
                // 1. 记录 ProhibitSwimming (禁止游泳) 的原始状态
                bool origSwim = DR_IsModuleEnabled(accessory, "ProhibitSwimming");
                bool isVerified = origSwim; // 如果原本就是开着的，说明验证肯定通过了

                // 2. 发送测试加载指令
                DR_LoadModule(accessory, "ForceRootAndSilence"); // 测本地模块
                if (!origSwim)
                {
                    DR_LoadModule(accessory, "ProhibitSwimming"); // 代理测在线验证
                }
    ...
    _ = HandleP4PetrifyAsync(sa, isTrue, (int)dur, _runId, 0); 
        ...
        private float CalcP4PetrifySafeRot(ScriptAccessory sa, bool isTrue, uint bossId)
        {
            ...
            if (players.Count > 0)
            {
                ...
                if (isTrue) {
                    // 真石化（需背对）：必须在 45 度之外。用 50 度作为危险区，进入 50 度就强制扭头（容错高）
                    ...
                    // 伪石化（需面朝）：必须在 45 度之内。用 40 度作为安全区，超过 40 度必须强制扭头（防死）
