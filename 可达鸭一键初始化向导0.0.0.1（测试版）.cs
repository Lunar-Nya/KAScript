...
{
    [ScriptType(name: "可达鸭一键初始化向导", territorys: [], guid: "50fe62d3-88e1-3190-c4aa-9eddcb78d3e5", version: "0.0.0.1", author: "yuemao3", note: "测试版。实验性脚本，不保证长期可用。首次安装可达鸭的一键初始化配置脚本。可以按需分别一键调整语言、默认危险安全区的颜色、默认小队排序预设，自动添加在线库、配置网络代理，用完即自动卸载并删除。")]
    public class KodakkuInitWizard
    {
        ...
        private void OnDraw()
        {
            ...
            bool open = true;
            ImGui.SetNextWindowSize(new Vector2(540, 420), ImGuiCond.FirstUseEver);
            ImGui.PushStyleVar(ImGuiStyleVar.WindowRounding, 8f);

            if (ImGui.Begin("可达鸭初始化向导", ref open, ImGuiWindowFlags.NoCollapse))
            {
                ...
                RenderCheckItem("4. 在线库链", "自动添加在线脚本仓库", repoState);
                if (repoState < 2)
                {
                    ImGui.SameLine(repoState == 1 ? 480 : 355);
                    
                    if (repoState == 0)
                    {
                        if (ImGui.Button("仅常用在线库##commonrepo")) ExecuteReflectionConfig("CommonRepo");
                        ImGui.SameLine();
                    }
                ...
                if (ImGui.Button("关闭此窗口并自动卸载和删除本脚本", new Vector2(-1, 40)))
                {
                    open = false; 
                }
            }
