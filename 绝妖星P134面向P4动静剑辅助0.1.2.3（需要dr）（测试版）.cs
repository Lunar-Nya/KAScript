...
{
    [ScriptType(name: "绝妖星P134面向P4动静剑辅助", territorys: [1363], guid: "d4e5f6a1-c2b3-49d8-90ef-1a2b3c4d5e6f", version: "0.1.2.3", note: "萌新第一次写脚本。测试版，包含P1P3面向，及P4真伪石化、真伪炸弹自动处理，可能会电。类似功能的脚本尽量只开一个。\n依赖于DailyRoutines插件中的自动面向摄像机方向（在线验证模块）和禁止主动移动与释放技能（自制本地模块）。\n想要稳定使用的话请麻烦去可达鸭dc示例和分享中[月猫三]个人脚本分享里看下使用说明。", author: "yuemao3")]
    public class DMU_AutoFaceCombinedAssist
    {
        ...
        private void InitDRIpc(ScriptAccessory sa)
        {
            try 
            {
                ...
                {
                    sa.Method.SendChat("/e [IPC报错] 委托绑定失败！请确保DR版本正确。");
                    return;
                }
            ...
            {
                sa.Method.SendChat($"/e [IPC报错] 初始化异常：{ex.Message}");
            }
        }
        ...
        private void LogDebug(ScriptAccessory sa, Func<string> messageFactory) { 
            if (EnableDebugLog) sa.Method.SendChat($"/e [测试日志] {messageFactory()}"); 
        }
        ...
        private float CalcBossTrackingRot(ScriptAccessory sa, ulong bossId) 
        {
        ...
        private float CalcPlayerTargetRot(ScriptAccessory sa)
        {
            ...
            ulong targetId = myObj.TargetObjectId; 
        ...
        private async Task HandleP4PetrifyAsync(ScriptAccessory sa, bool isTrue, int dur, int runId, ulong bossId)
        {
            ...
            if (!await WaitUntilAsync(sa, lockTime, runId, null, () => DR_LockOnChara(sa, CalcBossTrackingRot(sa, bossId)))) return;
